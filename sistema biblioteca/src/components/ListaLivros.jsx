...
export default function ListaLivros({ livros, setLivros }) {
  ...
  async function enviarComentario(data, livro) {
    ...
    try {
      ...
      if (!resposta.ok) throw new Error("Erro ao incluir avaliação do livro...")
  ...
  function avaliarLivro(e, livro) {
    e.preventDefault(); // Evita navegação ao clicar no botão
    e.stopPropagation(); // Impede propagação do evento

    MySwal.fire({
      title: <span style={{ fontFamily: "Arial" }}>Avaliação: {livro.titulo}</span>,
      html: (
        <form onSubmit={handleSubmit((data) => enviarComentario(data, livro))}
              ...
              <div>
                ...
                <div style={{ marginTop: 8 }}>
                  {(livro.notas && livro.notas.length > 0) ? (
                    ...
                    <div><em>Sem avaliações ainda</em></div>
                  )}
                </div>

              </div>
              <button className="btn-avaliar" onClick={(e) => avaliarLivro(e, livro)}>Avaliar </button>
            </div>
          </Link>
        </div>
      ))}
